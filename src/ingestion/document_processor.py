...
class DocumentProcessor:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        logger.info(
            f"DocumentProcessor initialized (chunk_size={self.chunker.chunk_size})"
        )
    ...
    def process_directory(
        ...
        supported_extensions = self.config.get(
            "ingestion.supported_formats", ["pdf", "txt", "docx"]
        )
    ...
    def _load_document(self, file_path: Path) -> List[Document]:
        ...
        try:
            ...
            logger.error(
                f"Document loading failed: {file_path.name}", extra={"error": str(e)}
            )
            raise
