...
class OpenAIEmbedder(BaseEmbedder):
    ...
    def __init__(
        ...
        if model_name not in self.MODEL_DIMENSIONS:
            logger.warning(
                f"Unknown OpenAI model: {model_name}, dimension may be incorrect"
            )
    ...
    def embed_text(self, text: str) -> List[float]:
        ...
        try:
            response = self.client.embeddings.create(
                model=self.model_name, input=text.strip()
            )
    ...
    def embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(valid_texts), self.batch_size):
            ...
            try:
                ...
                logger.debug(
                    f"Generated {len(batch_embeddings)} embeddings (batch {i // self.batch_size + 1})"
                )
